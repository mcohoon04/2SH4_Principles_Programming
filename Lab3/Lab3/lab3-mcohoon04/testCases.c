...
//=================Question 2================================
	...
	void TestQ2_inputGradesAndCalculateFinal(CuTest *tc) {
		...
		//char actual[1000], expected[1000]; this was unused so I commented out
	...
	void TestQ2_allfunctionstest(CuTest *tc){

		//initialize variable size to be used to hold size of class list
		    int size;
		    //Run all functions
			student **list = create_class_list("students.txt", &size);
			input_grades("grades.txt", list,size);
			compute_final_course_grades(list,size);
			output_final_course_grades("outputGrades.txt", list, size);
			//sucessful
			withdraw(1200, list, &size);
			withdraw(9000, list, &size);
			//unsucessful
			withdraw(1, list, &size);

			int actual=1;
			int expected=1;
			int expected_id[3] = {4580, 6547, 7000};
			for(int i =0;i<size;i++){
			    if(list[i]->student_id!=expected_id[i]){
			        actual=0;
			    }
			}
		CuAssertIntEquals(tc, expected, actual);
...
//===========================================================
	CuSuite* Lab3GetSuite() {
		...
		SUITE_ADD_TEST(suite, TestQ2_allfunctionstest);
