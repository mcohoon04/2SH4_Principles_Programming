...
void testGetElementInvalid2() {
	...
	string actual ="Empty for Now";
	try {
			int output = f.getElement(4, 5);	// invalid indexes
	}
	catch (std::out_of_range& e){
			 actual = e.what();
	}
...
void testCopy2() {
	//change copy and test if origin remains same
	int row=4,col=5;
    int data[4][5] = {{1,2,3,4,5},{6,7,8,9,0},{0,0,1,2,3},{0,0,0,4,5}};
	int*input_data[4];
	for (int i=0;i<4;i++){
	...
	Matrix origin =  Matrix(row,col,input_data);
    Matrix copied = origin.copy(); // make a copy
    copied.setElement(0,0,0); //change copy
	string row1="1 2 3 4 5 \n", row2="6 7 8 9 0 \n", row3="0 0 1 2 3 \n",row4="0 0 0 4 5 \n";
	string expected=row1+row2+row3+row4;
    ASSERT_EQUAL(expected,origin.toString());
}
...
bool runAllTests(int argc, char const *argv[]) {
	...
	s.push_back(CUTE(testBaseConstructor));
	...
	s.push_back(CUTE(test2Constructor2));
